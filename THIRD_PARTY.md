...
This report records the UI references and runtime dependencies used by Base, along with the boundary between upstream packages and Base-owned source. It is an engineering inventory, not a legal conclusion. Any AGPL use or alternate licensing decision still requires legal review.
...
| [React Flow](https://github.com/xyflow/xyflow) | Workflow-canvas runtime dependency                      | Locked `@xyflow/react` `12.11.2`                | MIT           | Installed for Phase 3. React Flow provides viewport and interaction primitives; Base owns the controlled graph domain, adapters, validation, persistence, and history. Its required stylesheet is imported only by the workflow feature.                     |

## Source boundary

- No source files are copied from Plane, Propel, or React Flow; React Flow enters Base only through the locked MIT package dependency.
...
- React Flow's required stylesheet is scoped to the workflow feature, and Base-owned controlled graph adapters keep the workflow domain independent from the canvas library.

## Modified-source inventory

- **Copied source:** none. No Plane, Propel, Linear, or React Flow source files, assets, or generated output were copied into Base; React Flow's distributed package and required stylesheet remain governed by its MIT license.
- **New Base-owned source:** `apps/web/src/features/app-shell/**` and `apps/web/src/routes/_base*.tsx` contain the independent shell, workflow domain and adapters, fixtures, pages, route metadata, and tests.
...
No AGPL code enters the Base codebase. Adding Plane or Propel code, generated output, bundled assets, or runtime packages changes this boundary and requires an updated report plus legal review.
